...
parser.add_argument(
    '--triplet_margin', default='soft', type=common.float_or_string,
    help='What margin to use: a float value for hard-margin, "soft" for '
         'soft-margin, or no margin if "none".')

parser.add_argument(
    '--triplet_metric', default='euclidean', choices=loss.cdist.supported_metrics,
    help='Which metric to use for the distance between embeddings.')

parser.add_argument(
    '--triplet_loss', default='batch_hard', choices=loss.LOSS_CHOICES.keys(),
    help='Enable the super-mega-advanced top-secret sampling stabilizer.')

parser.add_argument(
    '--triplet_learning_rate', default=3e-4, type=common.positive_float,
    help='The initial value of the learning-rate (triplet), before it kicks in.')

parser.add_argument(
    '--lmcl_learning_rate', default=3e-4, type=common.positive_float,
    help='The initial value of the learning-rate (lmcl), before it kicks in.')

parser.add_argument(
    '--lmcl_alpha', default=0.40, type=common.positive_float,
    help='lmcl-alpha)')

parser.add_argument(
    '--lmcl_scale', default=64, type=common.positive_int,
    help='lmcl-scale)')

parser.add_argument(
    '--lmcl_dis_th', default=0.88, type=common.positive_float,
    help='lmcl-dis-th)')
...
def main():
    ...
    dists = loss.cdist(endpoints['emb'], endpoints['emb'], metric=args.triplet_metric)
    triplet_losses, train_top1, prec_at_k, _, neg_dists, pos_dists = loss.LOSS_CHOICES[args.triplet_loss](
        dists, pids, args.triplet_margin, batch_precision_at_k=args.batch_k-1)

    # Count the number of active entries, and compute the total batch loss.
    triplet_num_active = tf.reduce_sum(tf.cast(tf.greater(triplet_losses, 1e-5), tf.float32))
    triplet_loss_mean = tf.reduce_mean(triplet_losses)

    # Some logging for tensorboard.
    tf.summary.histogram('triplet_loss_distribution', triplet_losses)
    tf.summary.scalar('triplet_loss', triplet_loss_mean)
    tf.summary.scalar('triplet_batch_top1', train_top1)
    tf.summary.scalar('triplet_batch_prec_at_{}'.format(args.batch_k-1), prec_at_k)
    tf.summary.scalar('triplet_active_count', triplet_num_active)
    tf.summary.histogram('triplet_embedding_dists', dists)
    tf.summary.histogram('triplet_embedding_pos_dists', pos_dists)
    tf.summary.histogram('triplet_embedding_neg_dists', neg_dists)
    tf.summary.histogram('triplet_embedding_lengths',
                         tf.norm(endpoints['emb_raw'], axis=1))

    # LMCL
    num_classes = 576
    weight_decay = 1e-4
    lmcl_loss, lmcl_num_active = cos_loss_w_bh(endpoints['emb_2'], pids, num_classes,
                                               alpha=args.lmcl_alpha,
                                               scale=args.lmcl_scale,
                                               dis_th=args.lmcl_dis_th)

    regularization_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    reg_loss = weight_decay*sum(regularization_losses)
    lmcl_losses = lmcl_loss + reg_loss

    lmcl_loss_mean = tf.reduce_mean(lmcl_losses)


    # Some logging for tensorboard.
    tf.summary.histogram('lmcl_loss_distribution', lmcl_losses)
    tf.summary.scalar('lmcl_active_count', lmcl_num_active)
    tf.summary.scalar('lmcl_loss', lmcl_loss_mean)

    tf.summary.histogram('lmcl_embedding_lengths',
                         tf.norm(endpoints['emb_raw_2'], axis=1))
    ...
    if 0 <= args.decay_start_iteration < args.train_iterations:
        triplet_learning_rate = tf.train.exponential_decay(
            args.triplet_learning_rate,
            tf.maximum(0, global_step - args.decay_start_iteration),
            args.train_iterations - args.decay_start_iteration, 0.001)
        lmcl_learning_rate = tf.train.exponential_decay(
            args.lmcl_learning_rate,
            tf.maximum(0, global_step - args.decay_start_iteration),
            args.train_iterations - args.decay_start_iteration, 0.001)
    else:
        triplet_learning_rate = args.triplet_learning_rate
        lmcl_learning_rate = args.lmcl_learning_rate

    tf.summary.scalar('triplet_learning_rate', triplet_learning_rate)
    tf.summary.scalar('lmcl_learning_rate', lmcl_learning_rate)

    triplet_optimizer = tf.train.AdamOptimizer(triplet_learning_rate)
    lmcl_optimizer = tf.train.AdamOptimizer(lmcl_learning_rate, epsilon=0.1)

    # Update_ops are used to update batchnorm stats.
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        triplet_train_op = triplet_optimizer.minimize(triplet_loss_mean, global_step=global_step)
        lmcl_train_op = lmcl_optimizer.minimize(lmcl_loss_mean, global_step=global_step)
    ...
    with tf.Session(config=config) as sess:
        if args.resume:
            ...
            if args.initial_checkpoint is not None:
                model_variables = [v for v in model_variables if v.name.split('/')[1]!='attention_1']
                model_variables = [v for v in model_variables if v.name.split('/')[1]!='attention_2']
        ...
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u:
            for i in range(start_step, args.train_iterations):
                ...
                _, _, summary, step, b_embs, triplet_b_loss, lmcl_b_loss, b_fids = \
                    sess.run([triplet_train_op, lmcl_train_op, merged_summary, global_step,
                              endpoints['emb'], triplet_losses, lmcl_losses, fids])
                ...
                log.info('iter:{:6d}, triplet loss min|avg|max: {:.3f}|{:.3f}|{:6.3f}, '
                         'lmcl loss: {:.3f}, ETA: {} ({:.2f}s/it)'.format(
                             step,
                             float(np.min(triplet_b_loss)),
                             float(np.mean(triplet_b_loss)),
                             float(np.max(triplet_b_loss)),
                             float(np.mean(lmcl_b_loss)),
