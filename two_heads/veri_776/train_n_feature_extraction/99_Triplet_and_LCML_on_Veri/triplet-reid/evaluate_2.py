...
parser.add_argument(
    '--excluder', required=True, choices=('market1501', 'diagonal','duke', 'veri_776'),
...
parser.add_argument(
    '--query_embeddings_2', required=True,
    help='Path to the h5 file containing the query embeddings_2.')
...
parser.add_argument(
    '--gallery_embeddings_2', required=True,
    help='Path to the h5 file containing the gallery embeddings_2.')
...
parser.add_argument(
    '--metric_2', required=True, choices=loss.cdist.supported_metrics,
    help='Which metric to use for the distance between embeddings.')
...
def main():
    ...
    # Load the two datasets fully into memory.
    with h5py.File(args.query_embeddings_2, 'r') as f_query:
        query_embs_2 = np.array(f_query['emb_2'])
    with h5py.File(args.gallery_embeddings_2, 'r') as f_gallery:
        gallery_embs_2 = np.array(f_gallery['emb_2'])
    ...
    batch_pids, batch_fids, batch_embs, batch_embs_2 = tf.data.Dataset.from_tensor_slices(
        (query_pids, query_fids, query_embs, query_embs_2)
    ).batch(args.batch_size).make_one_shot_iterator().get_next()

    batch_distances_1 = loss.cdist(batch_embs, gallery_embs, metric=args.metric)
    batch_distances_2 = loss.cdist(batch_embs_2, gallery_embs_2, metric=args.metric_2)
 
    batch_distances = batch_distances_1 + batch_distances_2
    ...
    with tf.Session() as sess:
        for start_idx in count(step=args.batch_size):
            ...
            gallery_pids = np.array(gallery_pids, '|U')

            # Compute the pid matches
            pid_matches = gallery_pids[None] == pids[:,None]

            #print (gallery_pids.shape, pids.shape)
            #print (gallery_pids[:5])
            #print (pids[:5])
